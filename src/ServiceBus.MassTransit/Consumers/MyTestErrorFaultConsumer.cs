...
public class MyTestErrorFaultConsumer(ILogger<MyTestErrorFaultConsumer> logger)
    : IConsumer<Fault<MyTestErrorMessage>>
{
    public async Task Consume(ConsumeContext<Fault<MyTestErrorMessage>> context)
    {
        logger.LogError(
            "An error occurred processing message: {Id} {time} {exceptionMsg}",
